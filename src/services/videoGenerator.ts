...
export async function generateThumbnail(
  ...
  try {
    ...
    try {
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(dateTimeStr, dateBadgeX + (dateBadgeWidth / 2), dateBadgeY + (dateBadgeHeight / 2) + 7.2); // Moved down by 0.7px (from +6.5 to +7.2)
      ...
      ctx.fillText('CB', logoX + 2, logoY + 10.2); // Moved down by 0.7px (from +9.5 to +10.2)
      
      // Outline
      ctx.strokeStyle = '#ffffff';
      ctx.lineWidth = 3; // Doubled from 1.5
      ctx.strokeText('CB', logoX, logoY + 8.2); // Moved down by 0.7px (from +7.5 to +8.2)
      
      // Main text
      ctx.fillStyle = '#000000';
      ctx.font = 'bold 36px Arial'; // Doubled from 18px
      ctx.fillText('CB', logoX, logoY + 8.2); // Moved down by 0.7px (from +7.5 to +8.2)
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(badgeText, badgeX + (badgeWidth / 2), badgeY + (badgeHeight / 2) + 9.2); // Moved down by 0.7px (from +8.5 to +9.2)
...
<svg width="1280" height="720" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="55" y="90.7" font-family="Arial, sans-serif" font-size="36" font-weight="bold" fill="#000000" text-anchor="middle" dominant-baseline="middle" stroke="#ffffff" stroke-width="3">CB</text>
  ...
  <text x="320" y="91.7" font-family="Arial, sans-serif" font-size="34" font-weight="bold" fill="${textColorRgb}" text-anchor="middle" dominant-baseline="middle">${isDeepDive ? 'DEEP DIVE' : 'LATEST CRYPTO NEWS!'}</text>
  ...
  <text x="1086" y="89.7" font-family="Arial, sans-serif" font-size="30" font-weight="bold" fill="${textColorRgb}" text-anchor="middle" dominant-baseline="middle">${escapeXml(dateTimeStr)}</text>
</svg>`;
